...
export default function getTrainingAnswer({
  ...
  if (content.includes('is training in the river')) {
    if (content.includes(':normiefish:')) generateRows(RIVER, 'normie');
    if (content.includes(':goldenfish:')) generateRows(RIVER, 'golden');
    if (content.includes(':EPICfish:')) generateRows(RIVER, 'epic');
